...
BASE_LETTERS = ['e', 'w', 'i', 'v', 'r', 'q', 'x', 'p',
                'k', 'h', 'g', 'a', 'y', 'f', 's', 'o',
                'j', 'u', 'd', 'n', 'l', 'm',
                'c', 'z', 'b', 't']
CAPITAL_LETTERS = ['B', 'N', 'H', 'P', 'I', 'X', 'C', 'D',
                   'K', 'T', 'U', 'V', 'J', 'G', 'M', 'Y',
                   'R', 'Z', 'S', 'E', 'A', 'O',
                   'Q', 'L', 'W', 'F']
...
def password_length():
    ...
    Try will validate if the input is an int and if the
    int is between 8 and 40
    """
...
def validate_binary_question(user_answer):
    """
    Checks if the users answer to Yes/No question is valid given that it"
    "must be Y or N"
    """
...
def user_password_requirements(type):
    ...
    """
    while True:
        user_choice = input(f"Do you want to use {type} in your pass"
...
def user_password_pref():
    """
    Asks user if they want symbols, capital letters and numbers
    in their password returns
    """
    password = []
...
def main():
    ...
    change the password using the same parameters or
    create a completely new set of parameters
    """
    while True:
        ...
        while True:
            ...
            while True:
                user_choice = input("Would you like to generate a "
                                    "new password using the same parameters?"
                                    "\nPlease answer with 'Y' OR 'N'\n\n")
                if validate_binary_question(user_choice):
                    break
